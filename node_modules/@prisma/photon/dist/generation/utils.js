...
function getSelectReturnType({ name, actionName, renderPromise = true, hideCondition = false, isField = false, fieldName, }) {
    ...
    if (isList || hideCondition) {
        ...
        const renderType = (projection) => `${promiseOpen}${listOpen}${getPayloadName(name, projection)}<Extract${getModelArgName(name, projection, actionName)}<T>>${listClose}${promiseClose}`;
...
? ${renderType(Projection.include)} : ${promiseOpen}${listOpen}${name}${promiseClose}${listClose}`;
    ...
    const selectType = `${renderPromise ? 'Promise<' : ''}${getPayloadName(name, Projection.select)}<Extract${selectArgName}<T>>${renderPromise ? '>' : ''}`;
    const includeType = `${renderPromise ? 'Promise<' : ''}${getPayloadName(name, Projection.include)}<Extract${includeArgName}<T>>${renderPromise ? '>' : ''}`;
