...
function resolveBuilderConfig(config) {
    ...
    var defaultTypesPath = path.join(__dirname, "../../@types/nexus-typegen/index.d.ts");
    var defaults = {
        outputs: {
    ...
    var internalConfig = tslib_1.__assign({}, config, { outputs: {} });
    var shouldGenerateArtifacts = config.shouldGenerateArtifacts !== undefined
...
function buildTypes(types, config, schemaBuilder) {
    ...
    var internalConfig = resolveBuilderConfig(config || {});
...
function makeSchemaInternal(config, schemaBuilder) {
    var _a = buildTypesInternal(config.types, config, schemaBuilder), typeMap = _a.typeMap, dynamicFields = _a.dynamicFields, rootTypings = _a.rootTypings, missingTypes = _a.missingTypes;
...
function makeSchema(config) {
    var internalConfig = resolveSchemaConfig(config);
    var _a = makeSchemaInternal(internalConfig), schema = _a.schema, missingTypes = _a.missingTypes;
    if (internalConfig.outputs.schema || internalConfig.outputs.typegen) {
        // Generating in the next tick allows us to use the schema
        // in the optional thunk for the typegen config
        new typegenMetadata_1.TypegenMetadata(internalConfig).generateArtifacts(schema).catch(function (e) {
            console.error(e);
        });
    }
...
function generateSchema(config) {
    return tslib_1.__awaiter(this, void 0, void 0, function () {
        var internalConfig, _a, schema, missingTypes;
        return tslib_1.__generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    internalConfig = resolveSchemaConfig(config);
                    _a = makeSchemaInternal(internalConfig), schema = _a.schema, missingTypes = _a.missingTypes;
                    assertNoMissingTypes(schema, missingTypes);
                    return [4 /*yield*/, new typegenMetadata_1.TypegenMetadata(internalConfig).generateArtifacts(schema)];
                case 1:
                    _b.sent();
